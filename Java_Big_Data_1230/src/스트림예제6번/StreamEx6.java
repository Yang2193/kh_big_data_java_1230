...
// 스트림 최종 연산 : 중개 연산을 통해 변환 된 스트림은 마지막으로 최종 연산을 통해 요소를 소모해 결과를 출력
public class StreamEx6 {
    public static void main(String[] args) {
        ...
        OptionalInt reduce = stream3.reduce((s1, s2) -> s1 * s2); // 연산은 만들면 됨.
        System.out.println(reduce.getAsInt());

        Optional<String> rst1 = stream1.reduce((s1, s2) -> s1 + "+" + s2); // 계속 이어붙이는 것.
        //결과적으로 전부 더 하는 것과 같다.
        rst1.ifPresent(System.out::println);//ifPresent 존재한다면 찍어라
        ...
        OptionalInt rst2 = stream4.sorted().findFirst(); //Optional -> NullPointerException을 막는 방법 중 하나.
        System.out.println(rst2.getAsInt());
        OptionalInt rst3 = stream5.sorted().findAny(); // 병렬처리 관련 내용이 포함되어 있어서 요소의 첫번째값이 아닐 수도 있음.
        ...
        System.out.println(stream9.count()); // 요소의 개수
        System.out.println(stream10.max().getAsInt()); // max()는 가장 큰거 min() 가장 작은거

        //요소의 연산
        IntStream stream11 = IntStream.of(30, 90, 70, 10);
        DoubleStream stream12 = DoubleStream.of(30.3, 90.9, 70.11, 10,12);
        System.out.println(stream11.sum()); // 합
        System.out.println(stream12.average()); // 평균
